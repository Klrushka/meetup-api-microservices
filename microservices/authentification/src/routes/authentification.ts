...
export function initRoutes(): Router {
  ...
  router.post(
    ...
    expressAsyncHandler(authController.registration)
  );
  router.post(
    '/login',
    requestLoggerMiddleware,
  ...
  router.get(
    '/google',
    requestLoggerMiddleware,
    passport.authenticate(strategies.google, { scope: ['email', 'profile'] })
  );
  router.get(
    '/',
    requestLoggerMiddleware,
  ...
  router.get(
    '/verify-email/:token',
    requestLoggerMiddleware,
    expressAsyncHandler(authController.emailVerification)
  );
  router.post(
    '/verify-user',
    requestLoggerMiddleware,
    authController.isUserValid
  );
  return router;
}
